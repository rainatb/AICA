...
def get_latest_sensor_data():
    ...
    while arduino.in_waiting > 0:
        ...
        if "Tommy is squeezing" in line:
            latest_grip_status = line
        elif "Heart rate" in line:
            # add in manual readings of bpm and grip force into python to show that the message to the LLM changes with BPM
...
def generate_response(patient_name, message, patient_data, sensor_message):
    ...
    if sensor_message:
        if "squeezing" in sensor_message.lower():
            print("Take a deep breath.")
            context += " The patient is showing **high anxiety** (elevated grip force detected). Prioritize strong reassurance. Avoid distractions. Tell the patient to take a deep breath and relax."
        elif "elevated" in sensor_message.lower():
            print("Take a deep breath.")
            context += " The patient’s pulse is also high, indicating **strong anxiety**. Keep responses **extra short, reassuring calm**. Avoid overwhelming the patient with details.Tell the patient to take a deep breath and relax. "
        elif "normal" in sensor_message.lower() and "pulse is normal" in sensor_message.lower():
